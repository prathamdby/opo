#!/usr/bin/env node
import{writeFileSync as p,mkdirSync as l,readFileSync as h,existsSync as d}from"node:fs";import{join as i}from"node:path";import{execSync as m}from"node:child_process";var a=`name: opo agent
  ...
  agent:
    ...
    steps:
      ...
      - name: Install OpenCode
        ...
        run: |
          gh release download --repo anomalyco/opencode \\
      ...
      # ── 6b. Generate PR title and body from the diff ─────────────────────
      - name: Generate PR metadata
        ...
        run: |
          opencode run \\
            "Run: git diff HEAD | cat
             Analyze the diff. Then write the file .opo-pr.json with this exact JSON structure:
             {\\"title\\": \\"<conventional commit title>\\", \\"body\\": \\"<PR body>\\"}
             Title rules: format is 'type: description', max 50 characters,
             types are feat/fix/refactor/docs/test/chore/style/perf, no scope, imperative mood.
             Body rules: mandatory blank line after the title, then bullet points starting with '-',
             capitalize first word of each bullet, no periods, no blank lines between bullets.
             Write only .opo-pr.json. Do not modify any other file."

          if [ -f .opo-pr.json ] && jq -e . .opo-pr.json > /dev/null 2>&1; then
            PR_TITLE=$(jq -r '.title' .opo-pr.json | cut -c1-256)
            DELIM=$(openssl rand -hex 8)
            {
              echo "pr_body<<\${DELIM}"
              jq -r '.body' .opo-pr.json
              printf '\\\\n\\\\n---\\\\ *Opened by [opo](https://github.com/prathamdby/opo)*\\\\n'
              echo "\${DELIM}"
            } >> "$GITHUB_OUTPUT"
          else
            PR_TITLE=$(echo "$TASK" | head -n1 | cut -c1-256)
            DELIM=$(openssl rand -hex 8)
            {
              echo "pr_body<<\${DELIM}"
              printf 'See the diff for changes.\\\\n\\\\n---\\\\ *Opened by [opo](https://github.com/prathamdby/opo)*\\\\n'
              echo "\${DELIM}"
            } >> "$GITHUB_OUTPUT"
          fi
      ...
      - name: Ensure opo label exists
        ...
        run: |
          gh label create "opo" \\
            --color "0075ca" \\
            --description "Opened by the opo agent" \\
            2>/dev/null || true
      ...
      - name: Open pull request
        ...
        env:
          GH_TOKEN: \${{ secrets.GITHUB_TOKEN }}
          PR_TITLE: \${{ steps.pr-meta.outputs.pr_title }}
          PR_BODY: \${{ steps.pr-meta.outputs.pr_body }}
      ...
      - name: Cleanup branch on failure
        ...
        run: |
          git remote set-url origin \\
            "https://x-access-token:\${GH_TOKEN}@github.com/\${{ github.repository }}.git"
          git push origin --delete "$BRANCH" 2>/dev/null || true`,c=`{
...
- Follow existing code conventions in the repository. Match the style of surrounding code.`,n=".opencode/";var r=process.cwd();function s(e,t){let o=i(r,e);p(o,t,"utf8"),console.log(`  wrote  ${e}`)}function g(){try{return m("git remote get-url origin",{stdio:["pipe","pipe","pipe"]}).toString().trim().replace(/^git@github\.com:/,"").replace(/^https:\/\/github\.com\//,"").replace(/\.git$/,"")}catch{return"<owner>/<repo>"}}try{l(i(r,".github","workflows"),{recursive:!0}),s(".github/workflows/agent.yml",a),s("opencode.jsonc",c),s("AGENTS.md",u);let e=i(r,".gitignore"),t=d(e)?h(e,"utf8"):"";if(!t.includes(n))p(e,t+(t.endsWith(`
`)?"":`
`)+n+`
`,"utf8"),console.log("  patched .gitignore");let o=g();console.log(`
